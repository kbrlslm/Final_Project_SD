...
def send_file():
    ...
    while cam.isOpened():
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            kk = cv2.rectangle(
                ll, (left, top), (right, bottom), (0, 0, 255), 2)
            ...
            for file in os.listdir():
                if '.png' in file:
                    ...
                    client.upload_file(
                        file, upload_file_bucket, upload_file_key)
                    os.remove(file)
...
def run_file():
    ...
    for file in os.listdir():
        if '.jpg' in file:
            ...
            upload_file_key = str(file)
            global x
            generate_predictions_with_pre_trained_model(
                upload_file_key, model, predictedPerson=x)
            send_email(x, file)
            os.remove(file)
